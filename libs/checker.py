...
class ProxyChecker:
    ...
    def add(self, proxy: str) -> bool:
        parsed: Optional[ParseResult] = self._parse_proxy(proxy)
        if parsed is None:
            return False
    ...
    def remove(self, proxy: str) -> bool:
        ...
        with self._lock:
            removed |= normalized in self._valid and not self._valid.discard(normalized)
            removed |= normalized in self._invalid and not self._invalid.discard(normalized)
            removed |= normalized in self._pending and not self._pending.discard(normalized)
    ...
    def _worker(
        ...
        while True:
            ...
            creds: Optional[tuple[str,str]] = self._creds.get(proxy)
            ok: bool = False
            try:
                if scheme == "http":
    ...
    # -----------------------
    @staticmethod
    def _parse_proxy(proxy: str) -> Optional[ParseResult]:
        try:
    ...
    def _check_http_connect_tls_proxy(
        ...
        try:
            ...
            resp: Optional[bytes] = self._recv_until(s, b"\r\n\r\n", timeout)
            ...
            if test_use_tls:
                ...
                tls: ssl.SSLSocket = ctx.wrap_socket(s, server_hostname=dest_host)
                req: str = (
                ...
                data: Optional[bytes] = self._recv_some(tls, timeout)
                ...
                data: Optional[bytes] = self._recv_some(s, timeout)
                return bool(data and b"HTTP/" in data)
    ...
    def _check_socks5_proxy(
        ...
        try:
            ...
            if creds:
                ...
                req = bytearray([0x01, len(u_bytes)]) + u_bytes + bytearray([len(p_bytes)]) + p_bytes
                s.sendall(req)
                ...
                resp: Optional[bytes] = self._recv_exact(s, 2, timeout)
                if not resp or resp[0] != 0x05 or resp[1] != 0x00:
                    return False

            # CONNECT request: VER=5, CMD=1 (CONNECT), RSV=0, ATYP=3 (domain),DST.ADDR, DST.PORT
            host_bytes: bytes = dest_host.encode("idna")
            req: bytearray = bytearray([0x05, 0x01, 0x00, 0x03, len(host_bytes)])
            req.extend(host_bytes)
            req.extend(dest_port.to_bytes(2, byteorder="big"))
            s.sendall(req)

            # SERVER reply: VER, REP, RSV, ATYP, BND.ADDR, BND.PORT 
            header: Optional[bytes] = self._recv_exact(s, 4, timeout)
    ...
    @staticmethod
    def _recv_some(sock: socket.socket, timeout: float, max_bytes: int = 4096) -> Optional[bytes]:
        try:
    ...
    @staticmethod
    def _recv_until(sock: socket.socket, marker: bytes, timeout: float, max_bytes = 65536) -> Optional[bytes]:
